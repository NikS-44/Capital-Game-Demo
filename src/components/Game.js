...
function Game(props) {
	...
	const [currentCapitalIndex, setCurrentCapitalIndex] = useState(
		getRandomInt(countriesList.length),
	);
	...
	const submitHandler = (event) => {
		...
		if (
			enteredCapital.toLowerCase() ===
			countriesList[currentCapitalIndex].capital
				.toLowerCase()
				.normalize("NFD")
				.replace(/[\u0300-\u036f]/g, "")
		) {
			...
			if (guessCounter > 1) {
				...
				setPlaceHolderText(
					`Try Again :(, you have ${guessesLeft} guess(es) left`,
				);
			} else {
				setEnteredCapital("");
				setPlaceHolderText(
					`Sorry, no more guesses left, the capital of ${countriesList[currentCapitalIndex].name} is ${countriesList[currentCapitalIndex].capital}`,
				);
				setGuessCounter(3);
				nextCountry();
			}
	...
	const removeCountryFromList = () => {
		setCountriesList((oldList) =>
			oldList.filter(
				(country) => country.name !== countriesList[currentCapitalIndex].name,
			),
		);
	};
	...
	const nextCountrySkip = () => {
		...
		setCurrentCapitalIndex(getRandomInt(countriesList.length));
		setPlaceHolderText(
			`The capital of ${countriesList[currentCapitalIndex].name} is ${countriesList[currentCapitalIndex].capital}`,
		);
	};
